...
# Обязательная подписка: ID канала (например -1001234567890). Если задан — доступ к боту только после подписки.
# Бот должен быть админом в канале. Репетиторы и админ не проверяются.
SUBSCRIPTION_CHANNEL_ID = None
# Ссылка на канал для кнопки «Подписаться» (например https://t.me/YourChannel)
